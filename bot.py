...
bot = Bot("vbczoeuxlchkoimdnhlvpqldgiektalj")
target = "g0B8yXZ0de6cb7b2be1bfa66739244b6"
...
def alert(guid,user,link=False):
	...
	name = bot.getUserInfo(guid)["data"]["user"]["first_name"]
...
while True:
	...
	try:
		...
		learn = loads(open("learn.json","r",encoding="utf-8").read())
		...
		for msg in messages:
			try:
				if msg["type"]=="Text" and not msg.get("message_id") in answered:
					if not sleeped:
						for i in learn.keys():
							if i in msg.get("text"):
								bot.sendMessage(target, str(learn[i]), message_id=msg.get("message_id"))
						...
						if hasAds(msg.get("text")) and not msg.get("author_object_guid") in admins :
							...
							try:
								text = msg.get("text").strip("یادبگیر ").split(":")
								word = text[0] + " "
								answer = text[1]
								learnfile = open("learn.json","w",encoding="utf-8")
								learn[word] = answer
								dumps(learn)
								learnfile.write(learn)
								bot.sendMessage(target, "✅ ذخیره شد .", message_id=msg.get("message_id"))
							except:
